<template>
    ...
    <van-cell class="cell" >
      <template #title>
        ...
        <span>{{orderData.service.serviceName}}</span>
      </template>
      <template #default>
    ...
    <van-cell-group class="cell">
      ...
      <van-cell >
        ...
        <template #default>
          <div  @click="showStartTime=true">{{currentDate||"请选择就诊时间"}}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>
      <van-cell >
        ...
        <template #default>
          <div  @click="showCompanion=true">{{companionName||"请选择陪护师"}}
            <van-icon name="arrow" />
          </div>
        </template>
      </van-cell>
    ...
    <van-cell-group title="服务需求" class="cell">
      ...
      style="height:100px"
      v-model="form.demand" 
      placeholder="请简单描述您就诊的情况" />
    </van-cell-group>
    <van-button type="primary" class="submit" size="large" position="bottom" @click=submit>提交订单</van-button>
...
/*订单状态进度条*/
//复用性逻辑，要写成组件(component)。createOrder.vue和detail.vue都会用到
