<template>
  <div>
    ...
    >
      <van-col span="6">
        <van-image width="100" height="90" :src="item.avatar_url"  radius="5"/>
      </van-col>
...
<script setup>
  ...
  /*--------------------首页数据--------------------*/
  const homeData=reactive({//为什么要定义初始值，如果不赋初始值，直接把后端数据赋值给homeData，reactive不能*对象整体*会丢失响应式。
  //因此要定义默认属性，给属性赋值
    hospitals:[],
    nav2s:[],
    navs:[],
  ...
  onMounted(async()=>{
    ...
    console.log('homeData=',homeData)//打印出来会啊是<proxy>{<target><handler>},这是因为打印的是一个reactive对象
